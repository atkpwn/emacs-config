...
* Miscellaneous
** Lorri & direnv
   I follow Max's blog [[https://maxtyler.net/blog/emacs-nix-environment][here]].
   #+begin_src emacs-lisp
     (use-package direnv
      :config
      (direnv-mode))
...
** Dictionary
   I follow the recommendation dictionary setup from [[https://www.masteringemacs.org/article/wordsmithing-in-emacs][this Mastering Emacs' article]].
   #+begin_src emacs-lisp
...
*** Fix emoji's height on macOS
    Emoji 💡's height is higher than normal character causing its line grows because of =eglot-code-action-indicator=, see [[https://www.reddit.com/r/emacs/comments/1n3o2we/on_macos_in_gui_with_eglot_turned_on_when_the/][this comment]].
    #+begin_src emacs-lisp
      (cond
...
*** Utilities
    The content of [[file:lisp/java-utils.el][java-utils]] was generated by Gemini 3 with a few small tweaks!
    #+begin_src emacs-lisp
